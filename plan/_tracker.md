...
| 3 — Audio | [phase-3-audio.md](phase-3-audio.md) | In progress | 2026-07-20 | |
...
- 2026-07-20 — Phase 3 started (Phase 2's human full-lap playtest still pending, but moving ahead per direct request): procedural Web Audio engine tone (`src/engine-audio.ts`) wired to live RPM/throttle telemetry, tied to the car's real V10 firing frequency rather than raw RPM. Verified headless (audio graph starts suspended and resumes on first real keypress, pitch/gain match the RPM/throttle formulas exactly at redline, mute toggle works, zero console errors). Remaining: human playtest of whether the tone actually helps judge shift points by ear (see phase file retrospective).
